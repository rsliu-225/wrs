...
def gen_h5_rec(f_name, org_path, goal_path, multiview=True):
    if not os.path.exists(goal_path):
        os.mkdir(goal_path)
    complete_pcds = []
    incomplete_pcds = []
    ...
    for fo in os.listdir(org_path):
        ...
        if fo in ['bspl', 'quad']:
            ...
            if f_name == 'train':
                ...
                id_range = range(0, max(id_list) + 1)
        print(id_range)
...
def gen_h5_conf(f_name, org_path, goal_path, multiview=True):
    if not os.path.exists(goal_path):
        os.mkdir(goal_path)
    pcds_gt = []
    pcds_i = []
    ...
    for fo in os.listdir(org_path):
        ...
        for f in os.listdir(os.path.join(org_path, fo, 'complete')):
            if f[-3:] == 'pcd' and int(f.split('_')[0]) in id_range:
                ...
                pcds_gt.append(np.asarray(gt, dtype='<f4'))
                # o3dpcd_gt.paint_uniform_color([0, 1, 0])
                # o3dpcd_i.paint_uniform_color([1, 0, 0])
                # o3d.visualization.draw_geometries([o3dpcd_gt, o3dpcd_i])
                labels.append(label)
...
if __name__ == '__main__':
    ...
    gen_h5_rec('train', org_path, goal_path, multiview=True)
    gen_h5_rec('test', org_path, goal_path, multiview=True)
    gen_h5_rec('val', org_path, goal_path, multiview=True)
    ...
    # gen_h5_conf('train', org_path, goal_path, multiview=False)
    # gen_h5_conf('test', org_path, goal_path, multiview=False)
    # gen_h5_conf('val', org_path, goal_path, multiview=False)
