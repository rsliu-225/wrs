...
if __name__ == '__main__':
    ...
    if os.path.exists(f"{config.ROOT}/log/exp/{exp_name}_state.pkl"):
        ...
        print("drawing state:", state, path_draw_sid)
        if path_draw_sid >= len(path_draw) - 1:
            ...
            # n0 = objmat4[:3, 0]
            # print(f, n0)
            # path_draw = mp_lft.refine_continuouspath_lft(path_draw, path_draw_sid, objrelpos, objrelrot, n0, grasp,
            #                                              pen_cm)
            # path_draw = mp_lft.refine_continuouspath_rgt(path_draw, path_draw_sid, objrelpos, objrelrot, f, grasp,
            #                                              pen_cm)
            # mp_lft.ah.show_animation_hold(path_draw, pen_cm, objrelpos, objrelrot)
            # base.run()
            '''
            ...
            state.append(skip_num / len(path_draw) * total_len)
            pickle.dump(state, open(f"{config.ROOT}/log/exp/{exp_name}_state.pkl", "wb"))
    else:
        path_draw_sid = 0
    ...
    refine draw path
    '''
    draw_primitive_armjnts = mp_lft.get_tool_primitive_armjnts(path_draw[0], objrelrot, length=30)
