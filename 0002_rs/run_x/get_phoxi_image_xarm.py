...
def get_img_rbt_opti(f_name, img_num, path='', jnt_range=(-np.pi, np.pi)):
    if not os.path.exists(os.path.join(config.ROOT, "img", path)):
        os.mkdir(os.path.join(config.ROOT, "img", path))
    phxi = phoxi.Phoxi(host=config.PHOXI_HOST)
    rbtx = xarmx.XArmShuidiX(ip="10.2.0.201")
    ...
    for a in np.linspace(jnt_range[0], jnt_range[1], img_num):
        ...
        if f_name != '':
            grayimg, _, _ = phxi.dumpalldata(f_name="img/" + path + "_".join([f_name, str(i).zfill(3)]) + ".pkl")
            pickle.dump(opti_data, open("img/" + path + "_".join([f_name, str(i).zfill(3), 'opti']) + ".pkl", 'wb'))
        else:
            grayimg, _, _ = phxi.dumpalldata(f_name="img/" + path + "_".join([str(i).zfill(3), 'opti']) + ".pkl")
            i += 1
            # cv2.imshow("grayimg", grayimg)
            # cv2.waitKey(0)
...
if __name__ == '__main__':
    folder_name = "plate_a_quadratic_2"
    f_name = ""
    img_num = 30
    # get_img_rbt(f_name, img_num, path=f'phoxi/seq/{folder_name}/')
    get_img_rbt_opti(f_name, img_num, path=f'phoxi/seq/{folder_name}/')
