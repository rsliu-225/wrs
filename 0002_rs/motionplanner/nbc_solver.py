...
class NBCOptimizerVec(object):
    ...
    def update_known(self, seedjntagls, nbv_pt, nbv_nrml, campos, ):
    ...
    def solve(self, seedjntagls, nbv_p, nbv_nrml, campos, method='SLSQP'):
        """
...
class NBCOptimizerMat4(object):
    def __init__(self, rbt, max_t=.01, max_a=np.pi / 6, max_dist=1, env=None, armname="arm", releemat4=np.eye(4),
                 toggledebug=TB):
        self.rbt = rbt
        self.armname = armname
        self.env = env
        ...
        self.goalrot, self.goaltrans = np.eye(3), np.zeros(3)
        self.init_eepos, self.init_eerot, self.init_eemat4 = None, None, None
    ...
    def objctive(self, x):
        ...
        eemat4 = rm.homomat_from_posrot(eepos, eerot)
        transmat4 = eemat4.dot(np.linalg.inv(self.init_eemat4))
        p_new = pcdu.trans_pcd(self.nbv_pts, transmat4)[0]
        ...
        self.wo_list.append(np.degrees(w_wo))
        self.obj_list.append(obj)

        return obj

    def update_known(self, seedjntagls, goaltrans, goalrot, campos, ):
        self.seedjntagls = seedjntagls
        self.init_eepos, self.init_eerot = self.rbt.get_gl_tcp()
        self.init_eemat4 = rm.homomat_from_posrot(self.init_eepos, self.init_eerot)
        self.goalrot, self.goaltrans, self.campos = goalrot, goaltrans, campos,
    ...
    def con_rot(self, x):
        ...
        eemat4 = rm.homomat_from_posrot(eepos, eerot)
        transmat4 = eemat4.dot(np.linalg.inv(self.init_eemat4))
        err = rm.axangle_between_rotmat(self.goalrot, transmat4[:3, :3])
        self.rot_err.append(err)
        return self.max_a - err

    def con_trans(self, x):
        self.rbth.goto_armjnts(x)
        eepos, eerot = self.rbt.get_gl_tcp()
        eemat4 = rm.homomat_from_posrot(eepos, eerot)
        transmat4 = eemat4.dot(np.linalg.inv(self.init_eemat4))
        err = np.linalg.norm(transmat4[:3, 3] - np.asarray(self.goaltrans))
        self.trans_err.append(err)
        return self.max_t - err
    ...
    def solve(self, seedjntagls, goaltrans, goalrot, campos, method='SLSQP'):
        """
        ...
        """
        time_start = time.time()
        self.update_known(seedjntagls, goaltrans, goalrot, campos)
        self.addconstraint(self.con_rot, condition="ineq")
        self.addconstraint(self.con_dist, condition="ineq")
        # self.addconstraint(self.con_diff_x, condition="ineq")
        # self.addconstraint(self.con_diff_y, condition="ineq")
