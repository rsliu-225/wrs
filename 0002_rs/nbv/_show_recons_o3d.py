...
if __name__ == '__main__':
    ...
    dist, _ = kdt_o.query(pcd_all, k=1)
    inds = [i for i, d in enumerate(dist) if d[0] < .005]
    print(len(pcd_all), len(inds))
    pcd_all_temp = np.asarray(pcd_all)[inds]

    pcd_o = inf.inference_sgl(np.asarray(pcd_all_temp))
    kdt_o = KDTree(pcd_o, leaf_size=2)
    dist, _ = kdt_o.query(pcd_all, k=1)
    inds = [i for i, d in enumerate(dist) if d[0] < .005]
    print(len(pcd_all), len(inds))
    ...
    # pts_nbv, nrmls_nbv, confs_nbv = pcdu.cal_nbv_pcn(pcd_all, pcd_o, radius=.02, icp=True, toggledebug=False)
    pts, nrmls, confs = pcdu.cal_conf(pcd_all, voxel_size=.005, theta=None)
    pts_nbv, nrmls_nbv, confs_nbv = pcdu.cal_nbv(pts, nrmls, confs)

    nbv_mesh_list = []

    kpts, kpts_rotseq = pcdu.get_kpts_gmm(pcd_all, show=False, rgba=(1, 1, 0, 1), n_components=12)
    inp_pseq = nu.nurbs_inp(kpts)
    # inp_pseq, inp_rotseq = du.get_rotseq_by_pseq(inp_pseq)
    inp_rotseq = pcdu.get_rots_wkpts(pcd_o, inp_pseq, show=False, rgba=(1, 0, 0, 1))
    ...
    if confs_nbv[0] != 1:
        o3d_circle = nu.gen_o3d_sphere(pts_nbv[0], radius=.002, rgb=[0, 1, 0])
    ...
    # o3dpcd_3 = o3dh.nparray2o3dpcd(pcd_cropped_list[2])
    ...
    # o3dpcd_3.paint_uniform_color(nu.COLOR[-1])
    o3dpcd_o.paint_uniform_color(nu.COLOR[2])
    nbv_mesh_list.append(o3dpcd)
    # nbv_mesh_list.append(o3dpcd_o)
