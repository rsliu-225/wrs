...
import utils.realsense as rs
import utils.vision_utils as vu
import utiltools.robotmath as rm
import utiltools.thirdparty.o3dhelper as o3dhelper
...
def get_pose_icp(pcd, objcm, use_rmse=False, downsampling_voxelsize=1, toggledebug=False):
    ...
    for angle in range(0, 181, 60):
        rot = rm.rodrigues((0, 0, 1), angle)
        inithomomat = rm.homobuild((0, 0, 0), rot)
...
if __name__ == '__main__':
    ...
    base = pc.World(camp=[2000, 0, 1000], lookatpos=[0, 0, 1000])
    base.pggen.genAxis()

    realsense = rs.RealSense()

    depthimglist, rgbimg_list = realsense.load_frame_seq("A_light.pkl")
    depthimg_bg = depthimglist[5]
    pcd_bg = vu.convert_depth2pcd(depthimg_bg)
    # pcdu.show_pcd(pcd_bg, rgba=(1, 1, 1, 0.1))
    ...
    for i, im in tqdm(enumerate(rgbimg_list)):
        ...
        pcd = vu.convert_depth2pcd(depthimg, toggledebug=False)
        if len(pcd) == 0:
