...
if __name__ == '__main__':
    ...
    for i in range(10, 50):
        ...
        plt.plot(range(len(goal_pseq))[1:-1], r_list_goal)
        plt.plot(res_pids[1:-1], r_list)
        plt.hlines(y=bconfig.R_BEND, xmin=0, xmax=len(goal_pseq), colors='gray', linestyles='--')
        plt.show()
        plt.plot(range(len(goal_pseq))[2:-1], torsion_list_goal)
        plt.plot(res_pids[2:-1], torsion_list)
        plt.show()
