...
class PiecewisePoly(object):
    ...
    def interpolate_by_max_jntspeed(self, path, control_frequency=.005, max_jntspeed=1.0472/15):
        """
        ...
        for i in range(self._n_pnts - 1):
            tmp_time_interval = max(abs(np.asarray(path[i + 1]) - np.asarray(path[i]))) / max_jntspeed
            samples = np.linspace(0,
                                  tmp_time_interval,
