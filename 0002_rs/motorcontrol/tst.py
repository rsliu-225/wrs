...
def sendToArduino(send_str):
    global startMarker, endMarker
    txLen = chr(len(send_str))
    adjSendStr = encodeHighBytes(send_str)
    adjSendStr = chr(startMarker) + txLen + adjSendStr + chr(endMarker)
    ser.write(adjSendStr.encode(encoding='utf-8'))
...
testData = []
testData.append('0005' + chr(specialByte) + '0100')
...
while n < numLoops:
    ...
    if ser.in_waiting == 0 and not waitingForReply:
