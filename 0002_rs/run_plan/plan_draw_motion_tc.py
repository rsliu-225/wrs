...
def dump_drawpath_ss(mp, motion_f_path, objmat4_list, grasp, grasp_id, msc=None, col_ps=None, threshold=1.0):
    ...
    if path_draw is not None:
        ...
        draw_dict[grasp_id] = [objrelpos, objrelrot, path_draw, time_cost]
...
if __name__ == '__main__':
    ...
    # motion_f_path = get_drawmotion_f_name(folder_path, drawpath_f_name, note='ik', type=type)
    ...
    #     objmat4_id = list(picknplace_pen_dict[grasp_id].keys())[0]
    #     objrelpos, objrelrot, path_cam2place_pen = picknplace_pen_dict[grasp_id][objmat4_id]
    #     objrelpos, objrelrot, path_picknplace_pen = picknplace_pen_dict[grasp_id]
    #     msc = path_picknplace_pen[-1]

    for grasp_id in range(0, 62):
