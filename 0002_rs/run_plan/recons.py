...
def crop_maker(img, pcd, tgt_id=None, show_frame=True):
    ...
    return ids[0], gripperframe, pcd_trans, \
           pcdu.crop_pcd(pcd_trans, x_range=(.05, .215), y_range=(-.4, .4), z_range=(-.2, -.0155))
...
def cal_nbc(folder_name, f_name, seedjntagls, seed=(.116, 0, -.1), gl_transmat4=np.eye(4), toggledebug=True):
    textureimg, depthimg, pcd = load_frame(folder_name, f_name)
    pcd = np.asarray(pcd) / 1000
    inx, gripperframe, pcd, pcd_cropped = crop_maker(textureimg, pcd)
...
if __name__ == '__main__':
    ...
    jnts = cal_nbc(folder_name=folder_name, f_name='000.pkl', seedjntagls=seedjntagls, gl_transmat4=gl_transmat4)
