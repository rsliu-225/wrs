...
def gen_partial_view(path, f, rot, rot_center, resolusion=(1280, 720),
    ...
    o3dpcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.005, max_nn=30))
    o3d.visualization.draw_geometries([o3dpcd], point_show_normal=True)
    print(np.arccos(np.asarray(o3dpcd.normals).dot(np.asarray([[0, 0, 1]]))))
...
def run_pcn(path, cat, f, cam_pos, o3dpcd_init, o3dpcd_gt, coverage_tor=.001, goal=.05, toggledebug=False):
    coverage = 0
    cnt = 0
    exp_dict = {}
    print(f'-----------pcn------------')
    ...
    json.dump(exp_dict, open(os.path.join(path, cat, 'res', f'pcn_{f.split(".ply")[0]}.json'), 'w'))


def run_pcn_nbv(path, cat, f, cam_pos, o3dpcd_init, o3dpcd_gt, coverage_tor=.001, goal=.95, toggledebug=False):
    coverage = 0
    cnt = 0
    exp_dict = {}
    print(f'-----------pcn+nbv------------')
    ...
    while coverage < goal and cnt < 5:
        ...
        print('coverage:', coverage)
    exp_dict['final'] = pcd_i.tolist()
    json.dump(exp_dict, open(os.path.join(path, cat, 'res', f'nbvpcn_{f.split(".ply")[0]}.json'), 'w'))


def run_nbv(path, cat, f, cam_pos, o3dpcd_init, o3dpcd_gt, coverage_tor=.001, goal=.95):
    coverage = 0
    cnt = 0
    exp_dict = {}
    print(f'-----------org------------')
    ...
    while coverage < goal and cnt < 5:
        ...
        print('coverage:', coverage)
    exp_dict['final'] = pcd_i.tolist()
    json.dump(exp_dict, open(os.path.join(path, cat, 'res', f'org_{f.split(".ply")[0]}.json'), 'w'))
...
if __name__ == '__main__':
    ...
    coverage_tor = .0015
    goal = .95
    if not os.path.exists(os.path.join(path, cat, 'res')):
        os.makedirs(os.path.join(path, cat, 'res'))
    for f in os.listdir(os.path.join(path, cat, 'mesh')):
        print(f'-----------{f}------------')
        if os.path.exists(os.path.join(path, cat, 'res', f'pcn_{f.split(".ply")[0]}.json')) and \
                os.path.exists(os.path.join(path, cat, 'res', f'org_{f.split(".ply")[0]}.json')):
            continue
        # o3dpcd_init = gen_partial_view(os.path.join(path, cat), f.split('.ply')[0], np.eye(3), [0, 0, 0],
        #                                rnd_occ_ratio_rng=(.2, .6), nrml_occ_ratio_rng=(.2, .6),
        #                                occ_vt_ratio=1, noise_vt_ratio=1, noise_cnt=random.randint(0, 5),
        ...
        o3dpcd_init = gen_partial_view(os.path.join(path, cat), f.split('.ply')[0], np.eye(3), [0, 0, 0],
                                       ...
                                       add_vt_occ=False, add_noise=False, add_rnd_occ=True, add_noise_pts=True,
                                       toggledebug=False)
        ...
        run_pcn(path, cat, f, cam_pos, o3dpcd_init, o3dpcd_gt, goal=goal, coverage_tor=coverage_tor)
